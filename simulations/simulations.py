...
def society_of_bandits(max_players: int = 20) -> List[AbstractAgent]:
    bandits = [AlegAATr(lmbda=0.0, ml_model_type='knn', enhanced=True), EXP4(), EEE(), UCB(), DUCB(), RUCB(), SWUCB(),
               DQNAgent(train_network=False), RAlegAATr(train_network=False), AleqgAATr(train_network=False),
               SMAlegAATr(enhanced=True), MADQN(train_networks=False)]
    agents = []
...
def simulations() -> None:
    ...
    n_iterations = N_EPOCHS * len(INITIAL_POP_CONDITIONS) * len(N_PLAYERS) * len(N_ROUNDS)
    ...
    n_cats_list = list(np.random.choice(a=[0, 1, 2], size=n_iterations, p=[0.9, 0.05, 0.05]))
                ...
                for n_rounds in N_ROUNDS:
                    ...
                    for opponents, opponents_label in list_of_opponents:
                        ...
                        agents_to_test.append(RAAT(enhanced=True,
                                                   track_vector_file=f'RAAT_{opponents_label}_pop={initial_pop_condition}_p={n_players}_r={n_rounds}_c={n_cats}.csv'))
