...
def generateIdealMatrix(relation_mat):
    ...
    for i in range(0, len(sign_mat)):
        for j in range(i, len(sign_mat)):
            sign_mat[i, j] = sign_mat[j, i] = -1 if sign_mat[i, j] == -1 or sign_mat[j, i] == -1 else 1 if sign_mat[
                                                                                                               i, j] == 1 or \
                                                                                                           sign_mat[
                                                                                                               j, i] == 1 else 0
...
class Node:
    ...
    def update(self, others, relation_mat):
        ...
        for other in others:
            ...
            if self.code != other.code and (ideal_mat[self.code, other.code] != 0.0 or ideal_mat[
                other.code, self.code] != 0.0 or self.min_margin > dist(self_pos, other_pos)):
                ideal_dist = ideal_mat[self.code, other.code]
                if ideal_dist < 1.0:
                    ideal_dist = self.min_margin

                x1_error = -2 * (ideal_dist - (
                        (self_pos[0] - other_pos[0]) ** 2 + (self_pos[1] - other_pos[1]) ** 2) ** 0.5) * (
                                   1.0 * self_pos[0] - 1.0 * other_pos[0]) * (
                                   (self_pos[0] - other_pos[0]) ** 2 + (self_pos[1] - other_pos[1]) ** 2) ** (-0.5)
                x2_error = -2 * (ideal_dist - (
                        (self_pos[0] - other_pos[0]) ** 2 + (self_pos[1] - other_pos[1]) ** 2) ** 0.5) * (
                                   1.0 * self_pos[1] - 1.0 * other_pos[1]) * (
                                   (self_pos[0] - other_pos[0]) ** 2 + (self_pos[1] - other_pos[1]) ** 2) ** (-0.5)
...
class NodeNetwork:
    ...
    def _graphPopularities(self, ax, fig, pops_arr, color_lookup=None, name_lookup={}, legend_colors={}):
        ...
        for n, node in enumerate(self.nodes[-1]):
            if color_lookup is None or node.name not in color_lookup:
            ...
            node_label = f'{name_lookup.get(node.name, node.name)}'
            line_width = 3 if node.name in color_lookup else 1
            ax.plot(np.arange(rounds), pops_arr[:, n], color=player_color, label=node_label, marker=marker,
            ...
            if color_lookup is not None:
                ax.annotate(name_lookup.get(node.name, node.name), (rounds - 1, pops_arr[-1, n]), xytext=(30, y_pos),
                            textcoords='offset points', arrowprops={'arrowstyle': '->'})

        ax.set_xticks(
            np.arange(rounds + 1 if rounds >= 10 and rounds % 2 == 1 else rounds, step=1 if rounds < 10 else 2))
    ...
    def graphExchange(self, ax, fig, exchanges, step=-1, color_lookup=None, name_lookup={}):
        ...
        for node in self.nodes[-1]:
            ...
            circle = plt.Circle(
                ...
                alpha=0.75, linewidth=line_width)
            ...
            ax.annotate("{}".format(name_lookup.get(node.name, node.name)),
                        (node.position[step][0], node.position[step][1]), xytext=(0, 0), textcoords='offset points',
                        va="center", ha="center", fontsize=7, c='k', weight='bold')
