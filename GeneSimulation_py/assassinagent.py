    ...
    def play_round(self, player_idx, round_num, received, popularities, influence, extra_data):
        ...
        if round_num == 0:
            ...
            if popularities[player_idx] >= 0.5:
                ...
                attack_power = (popularities[player_idx] / prop_assassin_attack) * self.gameParams["steal"] * \
    ...
    def update_vars(self, num_players, player_idx, influence):
        ...
        for i in range(0, num_players):
            if (i in self.the_assassins) and (
                    self.did_nono[i] or ((self.attacks_by[player_idx] > 0.0) and (self.attacks_by[i] == 0.0))):
