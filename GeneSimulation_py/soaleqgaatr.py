...
class SOAleqgAATr(AbstractAgent):
    def __init__(self, max_n_players: int = 30, learning_rate: float = 0.001, discount_factor: float = 0.9,
        ...
        self.aat_state, self.state = None, None
        ...
        aat_dim = 59
        self.state_dim = (max_n_players ** 2) + (2 * max_n_players)
        self.action_dim = len(self.generator_indices)
        self.model = DQN(aat_dim, self.state_dim)
        self.target_model = DQN(aat_dim, self.state_dim)
    ...
    def record_final_results(self, player_idx: int, round_num: int, received: np.array, popularities: np.array,
        ...
        if self.train_network:
            ...
            next_state_aat = np.array(self.generator_pool.assumptions(self.generator_to_use_idx).alignment_vector())
            next_state_aat = next_state_aat.reshape(1, -1)
    ...
    def play_round(self, player_idx: int, round_num: int, received: np.array, popularities: np.array,
        ...
        if self.train_network and np.random.rand() < self.epsilon:
            generator_to_use_idx = np.random.choice(self.action_dim)

        else:
            scaled_state = self.scaler.scale(next_state)
            q_vals_list = []

            for generator_idx in self.generator_indices:
                aat_vec = np.array(self.generator_pool.assumptions(generator_idx).alignment_vector()).reshape(1, -1)
                q_val = self.model((aat_vec, np.expand_dims(scaled_state, 0))).numpy()[0][0]
                q_vals_list.append(q_val)
            generator_to_use_idx = np.argmax(q_vals_list)

            if self.track_vector_file is not None:
                self.generator_to_use_idx = generator_to_use_idx
                network_state = self.model((aat_vec, np.expand_dims(scaled_state, 0)), return_transformed_state=True)
        ...
        curr_popularity = popularities[player_idx]
        next_state_aat = np.array(self.generator_pool.assumptions(generator_to_use_idx).alignment_vector())
        next_state_aat = next_state_aat.reshape(1, -1)
        if self.train_network and self.prev_popularity is not None:
            increase = curr_popularity - self.prev_popularity
            self.add_experience(self.generator_to_use_idx, increase, next_state_aat, next_state, False)
        if self.prev_popularity is None:
            self.scaler.update(next_state)
        self.prev_popularity = curr_popularity

        self.generator_to_use_idx = generator_to_use_idx
        self.aat_state, self.state = next_state_aat, next_state
    ...
    def add_experience(self, action: int, reward: float, next_state_aat: np.array, next_state: np.array,
                       done: bool) -> None:
    ...
    def save_network(self) -> None:
        # Save the network
        self.model.save('../GeneSimulation_py/soaleqgaatr_model/model.keras')

        with open('../GeneSimulation_py/soaleqgaatr_model/scaler.pickle', 'wb') as f:
            pickle.dump(self.scaler, f)

    def load_network(self) -> None:
        # Load the network
        self.model = keras.models.load_model('../GeneSimulation_py/soaleqgaatr_model/model.keras')
        self.scaler = pickle.load(open('../GeneSimulation_py/soaleqgaatr_model/scaler.pickle', 'rb'))
