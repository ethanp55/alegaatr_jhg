...
class AleqgAATr(AbstractAgent):
    def __init__(self, max_n_players: int = 30, learning_rate: float = 0.001, discount_factor: float = 0.9,
        ...
        aat_dim = len(self.generator_indices) * 59
        self.state_dim = (max_n_players ** 2) + (2 * max_n_players)
        self.action_dim = len(self.generator_indices)
        self.model = DQN(aat_dim, self.state_dim, self.action_dim)
        self.target_model = DQN(aat_dim, self.state_dim, self.action_dim)
    ...
    def record_final_results(self, player_idx: int, round_num: int, received: np.array, popularities: np.array,
        ...
        if self.train_network:
            ...
            next_state_aat = [self.generator_pool.assumptions(generator_idx).alignment_vector() for generator_idx in
                              self.generator_indices]
            next_state_aat = np.array(next_state_aat)
    ...
    def play_round(self, player_idx: int, round_num: int, received: np.array, popularities: np.array,
        ...
        if self.train_network and np.random.rand() < self.epsilon:
            ...
            if self.track_vector_file is not None:
                network_state = self.model((self.aat_state, np.expand_dims(scaled_state, 0)),
                                           return_transformed_state=True)
    ...
    def save_network(self) -> None:
        # Save the network
        self.model.save('../GeneSimulation_py/aleqgaatr_model/model.keras')

        with open('../GeneSimulation_py/aleqgaatr_model/scaler.pickle', 'wb') as f:
            pickle.dump(self.scaler, f)

    def load_network(self) -> None:
        # Load the network
        self.model = keras.models.load_model('../GeneSimulation_py/aleqgaatr_model/model.keras')
        self.scaler = pickle.load(open('../GeneSimulation_py/aleqgaatr_model/scaler.pickle', 'rb'))
...
#
#         # Model and target model
#         self.state_dim = (len(self.generator_indices) * 59) + (max_n_players ** 2) + (2 * max_n_players)
#         self.action_dim = len(self.generator_indices)
...
#                 [next_state.reshape(-1, 1), popularities.reshape(-1, 1), received.reshape(-1, 1)])
#
#             next_state = np.append(next_state_aat, next_state)
#
#             n_zeroes_for_state = self.state_dim - next_state.shape[0]
#             next_state = np.append(next_state, np.zeros(n_zeroes_for_state))
#
