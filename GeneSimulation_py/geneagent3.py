...
class GeneAgent3(AbstractAgent):
    ...
    def assassinGenes(self):
        ...
        self.genes["fixedUsage"] = 50  # Percentage of tokens the agent gives uniformly to its group
        self.genes["w_modularity"] = 100  # Weight of modularity in scoring potential communities
        self.genes["w_centrality"] = 50  # Weight of prominence in scoring potential communities
        self.genes[
            "w_collective_strength"] = 80  # Weight of target collective strength in scoring potential communities
        self.genes["w_familiarity"] = 50  # Weight of familiarity in scoring potential communities
        self.genes["w_prosocial"] = 70  # Weight of prosocial in scoring potential communities
        self.genes["initialDefense"] = 20  # Percentage of tokens to keep in round 1
        self.genes["minKeep"] = 20  # Minimum number of tokens the agent keeps in a round
        self.genes["defenseUpdate"] = 50  # How quickly the agent adapts its keeping based on new attacks on itself
        self.genes["defensePropensity"] = 50  # Degree to which the agent keeps tokens based on past attacks on itself
        self.genes["fearDefense"] = 0  # Degree to which the agent keeps tokens based on attacks on its friends
        self.genes[
            "safetyFirst"] = 0  # Whether the agent will prioritize defending or attacking if it doesn't have enough tokens to do both
        self.genes["pillageFury"] = 0  # Percentage of tokens to use in a pillage attack
        self.genes[
            "pillageDelay"] = 10  # Number of rounds to wait after the start of a game before considering pillaging
        self.genes["pillagePriority"] = 0  # Priority of pillage attacks
        self.genes["pillageMargin"] = 0  # The gain margin required before considering a pillage attack
        self.genes["pillageFriends"] = 0  # Whether the agent will consider pillaging its friends
        self.genes[
            "pillageCompanionship"] = 50  # Amount of help the agent initially believes it will get in a pillage attack over the first 5 rounds
        self.genes["vengenceMultiplier"] = 100  # How much vengeance to reciprocate
        self.genes["vengenceMax"] = 100  # Maximum number of tokens to use in a vengeance attack
        self.genes["vengencePriority"] = 100  # The priority of vengeance attacks
        self.genes["defendFriendMultiplier"] = 100  # How much to reciprocate in defend-friendly attacks
        self.genes["defendFriendMax"] = 100  # Maximum number of tokens to use in a defend-friendly attack
        self.genes["defendFriendPriority"] = 90  # Priority of defend-friendly attacks
        self.genes[
            "attackGoodGuys"] = 0  # Degree to which the agent will consider defend-friendly attacks against players that were not the first to attack and did not over-attack
        self.genes["limitingGive"] = 100  # Limit on how much the agent will give to a poor player
        self.genes[
            "groupAware"] = 0  # Degree to which the agent will consider defend-friendly attacks against players that belong to communities that have more collective strength than its own
    ...
    def defend_friend(self, player_idx, num_players, num_tokens, remaining_toks, popularities, influence,
        ...
        for i in selected_community:
            if (i == player_idx) or (self.me_importa[
                                         i] > 0.1):  # TODO: verify this threshold -- it is set arbitrarily without testing (seems okay for now)
                my_comm_vec[i] = 1.0
        ...
        for i in range(0, num_players):
            # Change on May 6
            if (self.govPlayer[i] == 1) or (i == player_idx) or (
                    (self.genes["attackGoodGuys"] < 50) and (self.bad_guys[i] < 0.2)) or (group_cat[i] == 2):
                continue
