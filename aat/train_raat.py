from GeneSimulation_py.raat import AATNetwork
import numpy as np
import os
import pickle
...
def train_raat(ENHANCED):
    ...
    for file in os.listdir(training_data_folder):
        ...
        if is_alignment_vectors:
            ...
            expanded_array = np.zeros((data.shape[0], 8))
    ...
    aat_vec_train = aat_scaler.fit_transform(aat_vec_train)
    aat_vec_val = aat_scaler.transform(aat_vec_val)
    with open(f'../aat/aat_network/aat_network_scaler{adjustment}.pickle', 'wb') as f:
        pickle.dump(aat_scaler, f)

    # Create the model
    assert aat_vec_train.shape[1] == 59 * 8
    model = AATNetwork(59 * 8)
    ...
    for epoch in range(N_EPOCHS):
        ...
        for i in range(n_batches):
            ...
            curr_aat_vec, curr_y = aat_vec_train[start_idx:end_idx, :], y_train[start_idx:end_idx, ]
        ...
        for i in range(n_val_batches):
            ...
            curr_aat_vec, curr_y = aat_vec_val[start_idx:end_idx, :], y_val[start_idx:end_idx, ]
        ...
        np.random.shuffle(indices)
        aat_vec_train, y_train = aat_vec_train[indices, :], y_train[indices,]
